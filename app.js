...
document.getElementById('sandbox-run').addEventListener('click', () => {
  ...
  if (lines.length < 2) { logEl.innerHTML = '<span class="log-warn">Need at least a header row and one data row.</span>'; return; }
  ...
  logs.push(`<span class="log-info">[INPUT] ${inCount} rows loaded, ${headers.length} columns: ${headers.join(', ')}</span>`);
  ...
  if (ops.trim) {
    rows = rows.map(row => { const r = {}; headers.forEach(h => { r[h] = row[h].trim(); }); return r; });
    logs.push('<span class="log-ok">[TRIM] Whitespace trimmed from all cells.</span>');
  }

  if (ops.lowercase) {
    rows = rows.map(row => { const r = {}; headers.forEach(h => { const v = row[h]; r[h] = isNaN(v) || v === '' ? v.toLowerCase() : v; }); return r; });
    logs.push('<span class="log-ok">[LOWERCASE] Text columns lowercased.</span>');
  }

  if (ops.nullify) {
    let nullCount = 0;
    rows = rows.map(row => { const r = {}; headers.forEach(h => { r[h] = row[h] === '' ? 'NULL' : row[h]; if (row[h] === '') nullCount++; }); return r; });
    logs.push(`<span class="log-ok">[NULLIFY] ${nullCount} empty cells replaced with NULL.</span>`);
  }

  if (ops.dedup) {
    const before = rows.length;
    const seen = new Set();
    rows = rows.filter(row => { const key = JSON.stringify(row); if (seen.has(key)) return false; seen.add(key); return true; });
  ...
  if (ops.negatives) {
    const revenueCol = headers.find(h => h.toLowerCase().includes('revenue') || h.toLowerCase().includes('amount') || h.toLowerCase().includes('price'));
    if (revenueCol) {
      let flagged = 0;
      rows = rows.map(row => { const v = parseFloat(row[revenueCol]); if (!isNaN(v) && v < 0) { row['_flag_negative'] = '⚠️ negative'; flagged++; } return row; });
      if (flagged > 0 && !headers.includes('_flag_negative')) headers.push('_flag_negative');
  ...
  if (ops.emailvalidate) {
    ...
    if (emailCol) {
      const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      let flagged = 0;
      rows = rows.map(row => { const v = row[emailCol]; if (v && v !== 'NULL' && v !== 'null' && !emailRe.test(v)) { row['_flag_email'] = '⚠️ invalid'; flagged++; } return row; });
      if (flagged > 0 && !headers.includes('_flag_email')) headers.push('_flag_email');
  ...
  const allHeaders = [...new Set([...headers, ...rows.flatMap(r => Object.keys(r))])];
  let html = '<table><thead><tr>';
  allHeaders.forEach(h => { html += `<th>${h}</th>`; });
  html += '</tr></thead><tbody>';
  rows.forEach(row => {
    html += '<tr>';
    allHeaders.forEach(h => { const v = row[h] || ''; const isFlag = h.startsWith('_flag'); html += `<td style="${isFlag ? 'color:#ffa94d' : ''}">${v}</td>`; });
    html += '</tr>';
  });
  html += '</tbody></table>';
